...
void vGPSTask(void *pvParam)
{
    ...
    for (;;)
    {
        ...
        if (gps_data.valid)
        {
            ...
            // Print GPS data
            usart_debug("LAT : %ld %c\r\n", gps_data.lat_i, gps_data.ns);
            usart_debug("LON : %ld %c\r\n", gps_data.lon_i, gps_data.ew);

            usart_debug("TIME: %02d:%02d:%02d\r\n",
                gps_data.hour,
                gps_data.minute,
                gps_data.second);

            usart_debug("DATE: %02d-%02d-%04d\r\n",
                gps_data.day,
                gps_data.month,
                gps_data.year);

            usart_debug("SPEED: %lu cm/s\r\n", gps_data.speed_cms);
        ...
        else
        {
            ...
            usart_debug("[GPS] NOT CONNECTED\r\n");

            prev_valid = 0;
        }

        //  IMPORTANT: small delay (poll fast)
        vTaskDelay(pdMS_TO_TICKS(100));
    }
}
